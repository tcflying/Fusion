...
/**
 * FNXC:HappierDirectSessionAttestation 2026-07-27-16:15:
 * Legacy Direct Session metadata must not own a third executable pin or trust
 * a command-chunk hash reported by the process being checked. It reuses the
 * canonical CLI path/version/source/hash attestation and backend resolver;
 * the historical local-extension manifest remains capability documentation,
 * never executable authority.
 */
export const verifyHappierDirectSessionRuntimeBuild = verifyHappierCliAttestation;
export type HappierDirectSessionRuntimeBuildVerification = HappierCliAttestation;
