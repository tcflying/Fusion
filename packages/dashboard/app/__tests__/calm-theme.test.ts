...
Calm is valid only when persistence, first-paint validators, selector metadata, tokens, and globally resolvable previews agree. This source contract catches a partial registration before a saved operator preference can flash or fall back.
*/
describe("Calm color theme", () => {
  ...
  it("keeps persisted, selector, and first-paint registries in exact order", () => {
    ...
    expect(CORE_COLOR_THEMES.filter((theme) => theme === "calm")).toHaveLength(1);
    expect(DASHBOARD_COLOR_THEMES).toContainEqual({
      value: "calm",
      label: "Calm",
      className: "theme-swatch-calm",
    });
  ...
  it("defines complete readable dark and light Calm token blocks", () => {
    const darkBlock = extractSelectorBlock(themeData, '[data-color-theme="calm"]');
    const lightBlock = extractSelectorBlock(themeData, '[data-color-theme="calm"][data-theme="light"]');
    ...
    expect(darkBlock).toContain("--bg: #111d2b;");
    expect(darkBlock).toContain("--accent: #8fc9b5;");
    expect(lightBlock).toContain("--bg: #f4f8fb;");
    expect(lightBlock).toContain("--accent: #2f7260;");
  });

  it("uses mode-specific global Calm preview properties for an unselected swatch", () => {
    const darkGlobals = extractSelectorBlock(themeData, ":root");
    const lightGlobals = extractSelectorBlock(themeData, '[data-theme="light"]');
    const darkSwatch = extractSelectorBlock(themeSelector, ".theme-swatch-calm");
    const lightSwatch = extractSelectorBlock(themeSelector, '[data-theme="light"] .theme-swatch-calm');

    for (const block of [darkGlobals, lightGlobals]) {
      for (const sample of [1, 2, 3, 4]) expect(block).toContain(`--calm-swatch-sample-${sample}:`);
    }
    for (const block of [darkSwatch, lightSwatch]) {
      for (const sample of [1, 2, 3, 4]) {
        expect(block).toContain(`--swatch-sample-${sample}: var(--calm-swatch-sample-${sample});`);
      }
