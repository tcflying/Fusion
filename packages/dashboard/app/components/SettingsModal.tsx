...
import { SETTINGS_SECTION_METADATA, type SettingsSectionMetadata } from "../../src/shared/settings-sections";
...
const RUNTIME_PLUGIN_SECTION_IDS: ReadonlyMap<string, string> = new Map([
  ...
  ["fusion-plugin-happier-runtime", "happier-runtime"],
...
const HAPPIER_RUNTIME_SECTION_METADATA: SettingsSectionMetadata = {
  id: "happier-runtime",
  label: "Happier",
  labelKey: "settings.nav.happierRuntime",
  scope: "global",
  group: "Integrations",
  advanced: true,
  searchableText: ["Happier runtime", "Codex session", "Claude session", "OpenCode session", "session bridge"],
};

/** The upstream registry is authoritative for built-ins; add the local runtime once until its plugin contributes metadata. */
const SETTINGS_SECTION_METADATA_WITH_HAPPIER: readonly SettingsSectionMetadata[] = (() => {
  if (SETTINGS_SECTION_METADATA.some((section) => section.id === HAPPIER_RUNTIME_SECTION_METADATA.id)) {
    return SETTINGS_SECTION_METADATA;
  }
  const afterHermesIndex = SETTINGS_SECTION_METADATA.findIndex((section) => section.id === "hermes-runtime");
  if (afterHermesIndex < 0) {
    return [...SETTINGS_SECTION_METADATA, HAPPIER_RUNTIME_SECTION_METADATA];
  }
  return [
    ...SETTINGS_SECTION_METADATA.slice(0, afterHermesIndex + 1),
    HAPPIER_RUNTIME_SECTION_METADATA,
    ...SETTINGS_SECTION_METADATA.slice(afterHermesIndex + 1),
  ];
})();

export const ADVANCED_SETTINGS_SECTION_IDS = new Set(
  SETTINGS_SECTION_METADATA_WITH_HAPPIER.filter((section) => section.advanced).map((section) => section.id),
);
...
export const SETTINGS_SECTIONS: SettingsSection[] = SETTINGS_SECTION_METADATA_WITH_HAPPIER.map((section) => ({
  id: section.id,
  label: section.label,
